import { ATLAS, ATLAS_MOZART, ATLAS_TRIAD, LOCAL, MLAB, OTHER } from '../modules/constants.js'
...
export const timeSinceLast = (user, collection) => {
  ...
  const last = findLast(user, collection)
...
export const getProcessMongo = () => {
  ...
  if (mongoUrl.indexOf('mongodb.net') > -1) {
    if (mongoUrl.indexOf('mozart') > -1) {
      return ATLAS_MOZART
    }
    if (mongoUrl.indexOf('Triad') > -1) {
      return ATLAS_TRIAD
    }
    return ATLAS
  }
